...
const CreateAdModal: React.FC<Props> = ({ data }) => {
  const [weekDays, setWeekDays] = useState<string[]>([])
  const [useVoiceChannel, setUseVoiceChannel] = useState(false)

  const handleCreatAd = useCallback(async (event: FormEvent) => {
        ...
        <form onSubmit={handleCreatAd} className='mt-8 flex flex-col gap-4'>
          ...
          <div className='flex gap-6'>
            ...
            <div className='flex flex-col gap-2 flex-1'>
              <label className='font-semibold' htmlFor="hoursStart">Qual horário do dia?</label>
